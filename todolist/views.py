...
def my_todos(request): 
    ...
    Task_ID_Business = TodoList.objects.filter(category__name="Рабочие", user=user)
    Task_ID_Personal = TodoList.objects.filter(category__name="Личные", user=user)
    Task_ID_Other = TodoList.objects.filter(category__name="Другие", user=user)
